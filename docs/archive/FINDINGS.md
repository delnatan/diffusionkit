...
### GLS fitting: does covariance-weighting actually beat truncation?

Produced by `scripts/validate_gls_recovery.py` (640 simulated Brownian
tracks, true alpha=1, D in [0.01, 0.25] um^2/s, sigma_loc=0.025 um,
track_length=60, dt=0.033s -- see README's "Classic, done properly" for
the method). Ground-truth simulation, so "better" here means closer to
the known true D/alpha, not just a different number.

1. **`D_gls_um2_s` beats `D_um2_s` (OLS-truncated) cleanly, at every D
   tested.** RMSE against true D: 0.0038->0.0028 at D=0.01 um^2/s,
   0.040->0.021 at D=0.10, 0.097->0.062 at D=0.25 (full table:
   `results/tables/validate_gls_recovery/recovery_summary_by_D.csv`).
   No caveat needed here -- the linear-space GLS fit needs no further
   approximation beyond the feasible-GLS pilot step (see README), and
   using (essentially) the whole track's covariance-weighted curve
   instead of `n_fit_points`'s ~10-point truncated window is a real,
   unambiguous improvement.

2. **`alpha_gls` is *not* a clean win at the same full-track range --
   this took a second pass to get right.** First attempt gave
   `fit_anomalous_diffusion_gls` the same "no fixed cap" range as the
   normal-diffusion fit. Result: alpha bias flipped sign and *grew* with
   D (-0.37 at D=0.01 to +0.13 at D=0.25) and alpha variance came out
   *higher* than plain OLS's in most rows -- a regression, not an
   improvement. Isolating the cause (single-track debug, D=0.1
   um^2/s, comparing GLS confined to OLS's own ~10-point window against
   GLS over the full ~58-point track):

   | estimator | mean alpha (truth=1) | std |
   | --- | --- | --- |
   | OLS, 10 pts | 0.936 | 0.135 |
   | GLS, same 10 pts | 1.036 | 0.124 |
   | GLS, full ~58 pts | 1.167 | 0.155 |

   The GLS *weighting* is good -- confined to the same window OLS uses,
   it beats OLS on both bias and variance. The regression comes
   specifically from extending the range: the log-log fit needs the
   TAMSD covariance converted into log-space via a delta-method
   linearization (`Var[log rho] ~= Var[rho]/rho^2`), valid only when a
   lag's *relative* fluctuation is small -- and it isn't, at the
   large-lag/few-pair tail this whole exercise exists to down-weight.
   The covariance still down-weights those points, just not by enough,
   and since alpha is a fitted *slope* (unlike D, a more local quantity)
   the residual mis-weighting drags the whole line. Fix (now the
   default in `fit_anomalous_diffusion_gls`, see its docstring):
   truncate to the same `n_fit_points`-style window as the OLS anomalous
   fit. Re-run with that fix: alpha_gls's bias magnitude is smaller than
   OLS's at every D tested but the highest one or two, and its variance
   is lower than OLS's at *every* D tested (full table, same CSV as
   above).
3. **Practical corollary: `n_points_used_gls` and `n_points_used_alpha_gls`
   are not the same window, on purpose**, unlike every other pair of
   `n_points_used*` columns in this table, which do share `n_fit_points`'s
   window. Read `TABLES.md`'s entries for both, not just one, before
   assuming they match.
4. **Explicitly accounting for the known localization noise (subtracting
   the offset, per finding 7 above) helps OLS but hurts log-linear GLS --
   and the fix is to stop linearizing, not to stop correcting.** Naive
   expectation: if offset-subtraction helps OLS alpha, it should help GLS
   alpha too. Tested by feeding `msd - offset` into
   `fit_anomalous_diffusion_gls` (same 640-track simulation as findings
   1-3). Result: bias got *worse* at every D tested (e.g. D=0.05: bias
   -0.053 uncorrected -> +0.156 corrected; D=0.25: +0.075 -> +0.130),
   consistently positive rather than trending toward zero with D. Cause:
   subtracting the offset shrinks the corrected MSD's magnitude --
   most at low D, where the offset *is* most of the raw signal -- which
   inflates the corrected curve's *relative* fluctuation right when the
   log-space delta-method covariance (`Var[log rho] ~= Var[rho]/rho^2`)
   needs that fluctuation to be small. Same root cause as finding 2, a
   second way to trigger it.

   Fix: `fit_anomalous_diffusion_nlgls` fits `MSD=4*K*tau^alpha` directly
   in linear space (Levenberg-Marquardt, weighted by the same linear
   `Sigma` the normal-diffusion GLS fit uses) -- no log, so no delta
   method, and `offset_um2` can be subtracted cleanly. Re-run with the
   offset-corrected nonlinear fit (`alpha_nlgls_corrected`) against the
   same ground truth, alongside a Bayesian MAP fit (`model="anomalous"`,
   default per-track localization-informed prior) on the *same* simulated
   tracks for reference:

   | true D | bias: OLS-corr | bias: NLGLS-corr | bias: Bayes | std: OLS-corr | std: NLGLS-corr | std: Bayes |
   | --- | --- | --- | --- | --- | --- | --- |
   | 0.01 | +0.036 | -0.013 | +0.035 | 0.213 | 0.203 | 0.172 |
   | 0.05 | -0.017 | -0.013 | -0.011 | 0.143 | 0.127 | 0.113 |
   | 0.10 | -0.033 | -0.022 | -0.021 | 0.187 | 0.152 | 0.136 |
   | 0.25 | -0.046 | -0.034 | -0.034 | 0.156 | 0.132 | 0.122 |

   `alpha_nlgls_corrected` matches Bayes's bias closely at every D tested
   and beats OLS-corrected on *both* bias and variance almost everywhere,
   at roughly 1/10th Bayes's wall-clock cost on the same tracks (1.7s vs
   16.4s for 640 tracks). It's now the recommended classic alpha estimate
   (`alpha_nlgls_corrected`, alongside `alpha_nlgls` uncorrected, same
   "always both" convention as the rest of this table) -- see README's
   "Classic, done properly".

5. **On real data, `alpha_nlgls_corrected` also has the lowest
   D-alpha correlation of any classic estimator -- confirming the
   simulated-bias reduction actually shows up as less confounding, not
   just a better number in a synthetic test.** `pearson r(D, alpha)`
   across the 365-track mobile-bead dataset (see finding 6 above for why
   this correlation is suspected to be a localization-noise artifact, not
   physics):

   | estimator | r(D, alpha) |
   | --- | --- |
   | `alpha` (OLS, uncorrected) | 0.728 |
   | `alpha_corrected` (OLS, offset-subtracted) | 0.590 |
   | `alpha_gls` (log-linear GLS, truncated) | 0.583 |
   | `alpha_nlgls_corrected` (nonlinear GLS, offset-corrected) | **0.531** |

   (`r(log10 D, alpha)`, a second cut of the same data used for the
   `K_jointplot*.png` figures, shows the same ordering more sharply:
   0.792 -> 0.619 -> 0.561.) Still far from zero -- this is a real,
   only-partially-fixed confound, not a solved problem, and `D_um2_s`
   itself is still the short-window OLS linear fit in this comparison
   (paired with each alpha estimator's *own* window, per
   `plot_K_jointplot`'s docstring) -- but each successive fix (offset
   correction, then removing the log-space approximation on top of it)
   removes a further, measurable slice of the correlation on real data,
   in the same direction the simulation predicted.
