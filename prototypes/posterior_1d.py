...
U = np.linspace(np.log(1e-4), np.log(10.), 501)
...
def _phi(x, on_time):
    """E|x + w| for w = difference of two uniform times in [0, on_time]."""
    x = np.abs(x)
    return x if on_time == 0 else x + np.clip(on_time - x, 0, None)**3 / (3 * on_time**2)


def motion_cov(m: int, dt: float, on_time: float = 0.) -> np.ndarray:
    """(m, m) covariance of m consecutive 1D displacements per unit D."""
    if not 0 <= on_time <= dt:
        raise ValueError("on_time must be between 0 and dt")
    k = np.arange(m)
    return toeplitz(_phi((k + 1) * dt, on_time) + _phi((k - 1) * dt, on_time) - 2 * _phi(k * dt, on_time))
...
def whiten(disp, sd, dt: float, on_time: float = 0.) -> Whitened:
    ...
    return Whitened(np.maximum(lam, 0.), V.T @ disp,
                    -.5 * (len(disp) * np.log(2 * np.pi) + 2 * np.log(np.diag(np.linalg.cholesky(B))).sum()))
...
def loglik(axes: list[Whitened], u: np.ndarray = U) -> np.ndarray:
    ...
    for lam, y, const in axes:
        ...
        total = total + const - .5 * np.sum(np.log(d) + y**2 / d, axis=1)
    return total


def track_loglik(positions, sd, dt: float, on_time: float = 0., u: np.ndarray = U) -> np.ndarray:
    ...
    return loglik([whiten(np.diff(x[:, a]), s[:, a], dt, on_time) for a in range(x.shape[1])], u)
...
def log_uniform(D_lo: float, D_hi: float, u: np.ndarray = U) -> np.ndarray:
    ...
    return np.where((u >= np.log(D_lo)) & (u <= np.log(D_hi)), 0., -np.inf)
...
def simulate(D: float, sd, dt: float, on_time: float, rng: np.random.Generator,
             axes: int = 2, sub: int = 100) -> np.ndarray:
    ...
    path = np.vstack([np.zeros(axes), np.cumsum(rng.normal(0, np.sqrt(2 * D * h), ((n - 1) * sub + n_on, axes)), axis=0)])
    w = np.ones(n_on + 1)
    w[[0, -1]] = .5
    blurred = np.array([w @ path[i * sub:i * sub + n_on + 1] / max(n_on, 1) if n_on else path[i * sub]
                        for i in range(n)])
...
def sequence(positions, sd, dt: float, log_prior: np.ndarray, on_time: float = 0.,
             u: np.ndarray = U) -> np.ndarray:
    """(m, len(u)); row k - 1 is the exact posterior after the first k displacements."""
    x = np.asarray(positions, float)
    return np.array([posterior(track_loglik(x[:k + 1], np.asarray(sd)[:k + 1], dt, on_time, u), log_prior)
...
def deconvolve(lls: np.ndarray, log_prior: np.ndarray, iters: int = 500, smooth: float = .5) -> np.ndarray:
    ...
    """
    L = np.exp(lls - lls.max(axis=1, keepdims=True))  # row scaling does not change the E-step
...
def summary(p: np.ndarray, u: np.ndarray = U, level: float = .9) -> dict[str, float]:
    ...
    return {"median": quantile(p, .5, u), "lo": quantile(p, (1 - level) / 2, u),
