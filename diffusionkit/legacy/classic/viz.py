...
def plot_K_jointplot(
    summary: pl.DataFrame, alpha_col: str = "alpha", d_col: str = "D_um2_s"
) -> plt.Figure:
    """Joint distribution of log10(D) and alpha across real-data tracks.

    D defaults to `D_um2_s` (units um^2/s) from `fit_normal_diffusion`: the
    *linear* MSD = 4*D*tau + b fit, over the same short-lag window
    (n_points_used) as the default alpha fit -- i.e. the short-time
    apparent D under a forced normal-diffusion assumption, deliberately
    paired against the independently-fit power-law exponent alpha from the
    *same* lag range. Pass `d_col="D_gls_um2_s"` to pair against the
    full-range GLS D estimate instead (`fitting.fit_normal_diffusion_gls`)
    -- a different, no-longer-same-window comparison, so a changed
    correlation there isn't apples-to-apples with the default pairing; the
    x-axis label states which D was used either way.

    Neither is `K_um2_s_alpha` (units um^2/s^alpha, from
    `fit_anomalous_diffusion`) -- D and K only coincide numerically when
    ...
    """
    df = (
        summary.filter(pl.col(d_col) > 0)
        .select(
            pl.col(d_col).log10().alias("log10_D"),
    ...
    # Not all d_col/alpha_col variants come from the same window or the same
    # kind of fit (alpha_nlgls* is nonlinear, in linear MSD space; D_gls_um2_s
    # is full-range) -- label each explicitly rather than assume the default
    # short-window-linear-D / log-log-alpha pairing this plot started as.
    _D_LABELS = {
        "D_um2_s": r"$\log_{10} D_{\mathrm{linear}}$  ($\mu m^2/s$, short-time normal-diffusion fit)",
        "D_gls_um2_s": r"$\log_{10} D_{\mathrm{GLS}}$  ($\mu m^2/s$, full-range covariance-weighted fit)",
    }
    _ALPHA_LABELS = {
        "alpha": r"$\alpha$ (log-log OLS fit)",
        "alpha_corrected": r"$\alpha$ (log-log OLS fit, offset-corrected)",
        "alpha_gls": r"$\alpha$ (log-log GLS fit)",
        "alpha_nlgls": r"$\alpha$ (nonlinear GLS fit, linear MSD space)",
        "alpha_nlgls_corrected": r"$\alpha$ (nonlinear GLS fit, offset-corrected)",
    }
    g.ax_joint.set_xlabel(_D_LABELS.get(d_col, f"log10({d_col})"))
    g.ax_joint.set_ylabel(_ALPHA_LABELS.get(alpha_col, alpha_col))
    g.ax_joint.legend(frameon=False, loc="upper left")
    same_window = d_col == "D_um2_s" and alpha_col in ("alpha", "alpha_corrected")
    g.ax_marg_x.set_title(
        f"Per-track D ({d_col}) vs. alpha ({alpha_col})  (r={r:.2f}, n={df.height}"
        + (f", {n_dropped} dropped: D<=0" if n_dropped else "") + ")\n"
        + (
            r"same short-lag window per track for both fits"
            if same_window
            else "NOT necessarily the same lag window per track for both fits -- see docstring"
        ),
...
def plot_ols_vs_gls_recovery(summary_with_truth: pl.DataFrame) -> plt.Figure:
    """OLS-truncated vs GLS recovery of D and alpha against known ground
    truth, for simulated Brownian tracks (see `scripts/validate_gls_recovery.py`).

    Top row: median fitted D (left) and its spread (right, IQR/median as a
    coefficient-of-variation proxy) vs true D. Bottom row: same for alpha,
    with a dashed line at the true value (alpha=1 for every simulated
    track). GLS sitting closer to true D/alpha with a narrower spread is
    the claim this plot is checking, not assuming.
    """
    fig, axes = plt.subplots(2, 2, figsize=(11, 9))

    truth_col = "true_D_um2_s"
    by_D = (
        summary_with_truth.group_by(truth_col)
        .agg(
            median_D_ols=pl.col("D_um2_s").median(),
            iqr_D_ols=(pl.col("D_um2_s").quantile(0.75) - pl.col("D_um2_s").quantile(0.25)),
            median_D_gls=pl.col("D_gls_um2_s").median(),
            iqr_D_gls=(pl.col("D_gls_um2_s").quantile(0.75) - pl.col("D_gls_um2_s").quantile(0.25)),
            median_alpha_ols=pl.col("alpha").median(),
            iqr_alpha_ols=(pl.col("alpha").quantile(0.75) - pl.col("alpha").quantile(0.25)),
            median_alpha_gls=pl.col("alpha_gls").median(),
            iqr_alpha_gls=(pl.col("alpha_gls").quantile(0.75) - pl.col("alpha_gls").quantile(0.25)),
        )
        .sort(truth_col)
    )
    true_D = by_D[truth_col].to_numpy()

    ax = axes[0, 0]
    ax.plot(true_D, by_D["median_D_ols"], "o-", color="darkorange", label="OLS (truncated)")
    ax.plot(true_D, by_D["median_D_gls"], "s-", color="steelblue", label="GLS")
    lo, hi = true_D.min() * 0.8, true_D.max() * 1.2
    ax.plot([lo, hi], [lo, hi], color="0.3", lw=1, ls="--", label="unity")
    ax.set_xscale("log"); ax.set_yscale("log")
    ax.set_xlabel(r"true D ($\mu m^2/s$)"); ax.set_ylabel(r"median fitted D ($\mu m^2/s$)")
    ax.set_title("D recovery: median"); ax.legend(frameon=False, fontsize=9)

    ax = axes[0, 1]
    ax.plot(true_D, by_D["iqr_D_ols"] / by_D["median_D_ols"], "o-", color="darkorange", label="OLS (truncated)")
    ax.plot(true_D, by_D["iqr_D_gls"] / by_D["median_D_gls"], "s-", color="steelblue", label="GLS")
    ax.set_xscale("log")
    ax.set_xlabel(r"true D ($\mu m^2/s$)"); ax.set_ylabel("IQR(D) / median(D)")
    ax.set_title("D spread (lower = more precise)"); ax.legend(frameon=False, fontsize=9)

    ax = axes[1, 0]
    ax.plot(true_D, by_D["median_alpha_ols"], "o-", color="darkorange", label="OLS (truncated)")
    ax.plot(true_D, by_D["median_alpha_gls"], "s-", color="steelblue", label="GLS")
    ax.axhline(1.0, color="0.3", lw=1, ls="--", label=r"true $\alpha=1$")
    ax.set_xscale("log")
    ax.set_xlabel(r"true D ($\mu m^2/s$)"); ax.set_ylabel(r"median fitted $\alpha$")
    ax.set_title(r"$\alpha$ recovery: median"); ax.legend(frameon=False, fontsize=9)

    ax = axes[1, 1]
    ax.plot(true_D, by_D["iqr_alpha_ols"], "o-", color="darkorange", label="OLS (truncated)")
    ax.plot(true_D, by_D["iqr_alpha_gls"], "s-", color="steelblue", label="GLS")
    ax.set_xscale("log")
    ax.set_xlabel(r"true D ($\mu m^2/s$)"); ax.set_ylabel(r"IQR($\alpha$)")
    ax.set_title(r"$\alpha$ spread (lower = more precise)"); ax.legend(frameon=False, fontsize=9)
