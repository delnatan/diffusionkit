...
def fit_population(
    ...
    fit range and each per-track OLS fit range (`fitting.n_fit_points`'s rule
    ...
    `per_track` also carries GLS fits (`D_gls_um2_s`, `alpha_gls`, ...) when
    `tracks` has the localization-noise columns needed for them -- see
    `fitting.fit_normal_diffusion_gls`/`fit_anomalous_diffusion_gls`. The
    ensemble-curve fits (`ensemble_normal_fit`/`ensemble_anomalous_fit`)
    stay OLS-only: `fitting.covariance`'s TAMSD covariance is derived for a
    single track's step process, and the ensemble curve mixes tracks of
    different lengths, which that derivation doesn't cover.
    """
